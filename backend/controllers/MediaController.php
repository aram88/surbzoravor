	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Media']))
		{
			...
			if($model->save()){
				...
				$post->read = 0;
				$post->visible =0;
				$post->path = $model->path;
				$post->type = 1;
                                $post->menu_id=25;
				$name = str_replace(' ', '-', trim($_POST['Media']['name'],' '));
				$post->slug = strtr($name,self::$translateString);
				$result = app()->db->createCommand()
                			->select('id')
                			->from('post')
                			->where('slug=:slug',array(':slug'=>$post->slug))
                			->queryAll();
				if (isset($result[0]['id'])){
					$post->slug.= rand(1,100000);
				}
				$post->name = $_POST['Media']['name'];
				$post->title= ucwords($_POST['Media']['name'].' | '.strtr(trim($_POST['Media']['name'],''),self::$translateString));
				$post->save();
