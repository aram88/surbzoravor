...
<div class="w100P  pull-left bg3 " >
	<div class="headerLink Bmargin5"></div>
	   <?php if ($data->type == 0):?>
		<strong><?php echo Chtml::link($data->name,'/post/view/'.$data->slug);?>	</strong>
		<?php endif?> 
		 <?php if ($data->type == 1):?>
		<strong><?php echo Chtml::link("<i class='fa fa-book fS16 red'></i> ".'   '.$data->name,$data->path,array('target'=>'_blank'));?>	</strong>
		<?php endif?> 
		<?php if ($data->type == 2):?>
		<strong><?php echo Chtml::link("<i class='fa fa-music fS16 red'></i> ".'   '.$data->name,'/post/view/'.$data->slug);?>	</strong>
		<?php endif?> 
	<div class="pull-right"><?php echo date("d/m/Y",strtotime($data->created_date))?></div>
	...
	<div class="span6 m0">
		...
		<?php if ($data->read): ?>
		    <?php if ($data->type == 0):?>
			<?php echo Chtml::link('Կարդալ ավելին  '."<span class='red'>>></span>",'/post/view/'.$data->slug);?>
			<?php endif;?>
			<?php if ($data->type == 1):?>
